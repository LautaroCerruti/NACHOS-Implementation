...
SyscallHandler(ExceptionType _et)
{
    ...
    switch (scid) {
...
#else 
            AddressSpace *space = new AddressSpace(executable, -1);
#endif
        ...
        case SC_MKDIR: {
            ...
            if (dirnameAddr == 0) {
                DEBUG('e', "Error: address to direname string is null.\n");
                machine->WriteRegister(2, -1);
                break;
            }
        ...
        case SC_CHDIR: {
            ...
            DEBUG('e', "`ChDir` request for name %s.\n", dirname);
        ...
        case SC_GETCWD: {
            int bufferUsr = machine->ReadRegister(4);
            int size = machine->ReadRegister(5);
            std::string path = currentThread->GetPath().GetPath();
...
#ifdef USE_TLB
static unsigned 
getVPN(unsigned vaddr) {
    return (unsigned) vaddr/PAGE_SIZE;
}
#endif
unsigned tlbSelection = 0;
...
{
#ifdef USE_TLB
    unsigned vpn = getVPN(machine->ReadRegister(BAD_VADDR_REG));

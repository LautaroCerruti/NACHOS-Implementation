...
Thread::Thread(const char *threadName, bool joineableParam)
{
    name     = threadName;
    stackTop = nullptr;
    stack    = nullptr;
    status   = JUST_CREATED;
    joineable = joineableParam;
    priority = 0;
    if (joineable)
        channel = new Channel();
#ifdef USER_PROGRAM
    ...
    spaceId = activeThreads->Add(this);
#endif
#ifdef FILESYS
...
#ifdef USER_PROGRAM
    activeThreads->Remove(spaceId);
    if(space != nullptr) {
        AddressSpace* aux = space;
        space = nullptr;
        delete aux;
    }
#endif
...
int
Thread::FileOpen(OpenFile* file) {
    int fid = files->Add(file);
    if (fid != -1) {
        fid += 2;
    }
    return fid;
...
OpenFile*
Thread::FileClose(int fid) {
    return files->Remove(fid-2);
}

OpenFile*
Thread::FileGet(int fid) {
    return files->Get(fid-2);
}
